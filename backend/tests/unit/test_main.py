...
class TestDeleteInvoice:
    ...
    def test_delete_invoice_storage_error(self, mock_supabase):
        ...
        # Should still succeed - storage failure shouldn't prevent database deletion
        assert response.status_code == 200
        data = response.json()
...
class TestSupabaseConnection:
    ...
    def test_get_single_invoice_without_supabase(self):
        ...
        assert response.status_code == 404
        data = response.json()
        assert "Demo mode" in data["detail"]
...
class TestEdgeCases:
    ...
    def test_upload_empty_file(self):
        ...
        # In demo mode, should accept the file but OCR processing may fail
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "uploaded"
